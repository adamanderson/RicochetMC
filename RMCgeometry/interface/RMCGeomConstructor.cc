...
detector(0), detectorPosition(G4ThreeVector(4.5*m, 0.*m, 1.5*m))	//detectorOffset(G4ThreeVector(3.5*m, 0., 1.5*m))
{
...
void RMCGeomConstructor::SetDetector(RMCVDetectorGeometry* theDet) {
	G4cout << "RMCGeomConstructor::SetDetector" << G4endl;
...
void RMCGeomConstructor::ConstructLab() {
    ...
    //new G4PVPlacement(0, lab->GetPosition(), logicalLab, "Lab", theOutside, false, 0);
    theWorld = new G4PVPlacement(0, lab->GetPosition(), logicalLab, "Lab", 0, false, 0);
...
void RMCGeomConstructor::ConstructDetector() {
    ...
    detector->SetPosition(detectorPosition);
