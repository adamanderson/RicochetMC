...
G4bool RMCZipSD::ProcessHits(G4Step* aStep, G4TouchableHistory* /*ROhist*/)
{
  ...
  G4cout << "totalenergydeposit = " << edep << G4endl;
  ...
  const G4VProcess* tparProc = postPoint->GetProcessDefinedStep();    
  if(tparProc) 
  zipHit->SetProcID(tparProc->GetProcessName());
  else
  zipHit->SetProcID("none");
  
  const G4VProcess* tProc = aStep->GetTrack()->GetCreatorProcess(); 
  if (tProc) 
  zipHit->SetParentProcID(tProc->GetProcessName());
  else
  zipHit->SetParentProcID("none");
